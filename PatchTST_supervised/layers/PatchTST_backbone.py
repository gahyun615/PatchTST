...
class PatchTST_backbone(nn.Module):
    ...
    def forward(
        ...
        if self.use_weekend_embedding and weekday_flag is not None:
            ...
            weekday_flag_patched = weekday_flag.unfold(
                dimension=-1, size=self.patch_len, step=self.stride
            )
            weekday_flag_patched = (
                weekday_flag_patched.float().mean(dim=-1).round().long()
            )
            weekday_flag_patched = torch.clamp(weekday_flag_patched, 0, 6)

        # model
        z = self.backbone(
            z, weekday_flag=weekday_flag_patched
...
class TSTiEncoder(nn.Module):  # i means channel-independent
    def __init__(
        ...
        **kwargs,
        ...
        # Day-of-week embedding (7 classes: 0=Mon, 1=Tue, ..., 6=Sun)
    ...
    def forward(
        ...
        if self.use_weekend_embedding and weekday_flag is not None:
            weekday_flag_expanded = weekday_flag.unsqueeze(1).repeat(1, n_vars, 1)
            weekday_flag_expanded = weekday_flag_expanded.reshape(
                -1, weekday_flag.shape[1]
            )
